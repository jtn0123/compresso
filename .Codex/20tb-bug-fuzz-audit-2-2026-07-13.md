...
| 11 | Peer-selected local path (fixed) | A peer could name an existing path outside the configured library and trick the master into copying that local file as the task result. | `test_remote_result_cannot_select_existing_file_outside_library`; result paths must be strict library descendants before local reuse. | Critical | Medium |
