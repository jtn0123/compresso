...
- Run `compresso doctor --role master --strict` on the authoritative scanner or
  ...
  arbitrary network targets and non-HTTPS linked addresses are rejected.
  Provide its API token through `COMPRESSO_DOCTOR_PEER_TOKEN`, or name another
  token variable with `--peer-token-env`. If `--output` is supplied, it must be
  a JSON filename stored under the user-data `readiness` directory.
...
- If readiness stays `503`, inspect `/config/.compresso/logs/compresso.log` for `STARTUP_VALIDATION_FAILED`, `STARTUP_READINESS_TIMEOUT`, `STARTUP_READINESS_PARTIAL_FAILURE`, or `UI_SERVER_STARTUP_FAILED`.
- If the root page does not load but readiness is healthy, inspect `/config/.compresso/logs/tornado.log` for routing or handler exceptions.
- If startup fails immediately, verify `/config` is writable, `/library` exists and is readable, and `/tmp/compresso` exists on writable fast storage.
...
- Start the container with your final `/config`, `/library`, and `/tmp/compresso` mounts.
...
- Stop the container cleanly if you see repeated `STARTUP_`, `UI_SERVER_`, or worker failure messages, then inspect `/config/.compresso/logs`.
...
1. Check readiness: `curl http://localhost:8888/compresso/api/v2/healthcheck/readiness` — wait for a `200` response.
2. Open the web UI at `http://localhost:8888/` and confirm the dashboard loads.
3. Inspect the startup summary: look for `STARTUP_SUMMARY` in `/config/.compresso/logs/compresso.log`.
4. Confirm `/library` contents match expectations (correct mount, expected files visible).
5. Confirm `/tmp/compresso` is writable with sufficient free space (`df -h /tmp/compresso` inside the container).
