...
class ExternalNotificationDispatcher(object, metaclass=SingletonType):
    ...
    def _send_to_channel(self, channel, event_type, context):
        ...
        try:
            if channel_type == 'discord':
                ...
                logger.warning("Unknown channel type '%s' for channel '%s'",
                               channel_type, channel.get('name', 'unnamed'))
        except Exception as e:
            logger.error("Failed to send %s notification to channel '%s': %s",
                         channel_type, channel.get('name', 'unnamed'), str(e))
    ...
    def _send_discord(self, channel, event_type, context):
        ...
        if resp.status_code >= 400:
            logger.warning("Discord webhook returned status %d for channel '%s'",
                           resp.status_code, channel.get('name', 'unnamed'))
    ...
    def _send_slack(self, channel, event_type, context):
        ...
        resp = requests.post(url, json=payload, timeout=10)
        if resp.status_code >= 400:
            logger.warning("Slack webhook returned status %d for channel '%s'",
                           resp.status_code, channel.get('name', 'unnamed'))
    ...
    def _send_webhook(self, channel, event_type, context):
        ...
        if resp.status_code >= 400:
            logger.warning("Webhook returned status %d for channel '%s'",
                           resp.status_code, channel.get('name', 'unnamed'))
    ...
    def test_channel(self, channel_config):
        ...
        try:
            ...
            logger.error("Test notification failed for channel '%s': %s",
                         channel_config.get('name', 'unnamed'), str(e))
