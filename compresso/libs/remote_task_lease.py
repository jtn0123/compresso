...
class RemoteTaskLease:
    ...
    @classmethod
    def _ensure_job_id(cls, task: Tasks) -> str:
        if task.job_id:
            return str(task.job_id)
        job_id = str(uuid.uuid4())
        updated = execute_count(Tasks.update(job_id=job_id).where((Tasks.id == task.id) & Tasks.job_id.is_null()))
        if updated:
            task.job_id = job_id
            return job_id
        persisted_job_id = Tasks.get_by_id(task.id).job_id
        if persisted_job_id is None:
            raise RuntimeError(f"Task {task.id} could not be assigned a job ID")
        return str(persisted_job_id)
    ...
    def heartbeat(
        ...
        expires_at = now + datetime.timedelta(seconds=ttl_seconds)
        updated = execute_count(
            Tasks.update(heartbeat_at=now, lease_expires_at=expires_at).where(
                (Tasks.id == task.id)
    ...
    def complete(
        ...
        if current.remote_result_checksum is not None:
            return bool(current.lease_token == token and current.remote_result_checksum == result_checksum)
        updated = execute_count(
            Tasks.update(
