# Compresso Frontend - Agent Guide

This document is designed to help AI agents understand the structure, technology stack, and development workflows of the Compresso Frontend project.
...
- **`src/`**: Source code root.
  ...
  - **`components/`**: Reusable Vue components. Organized by purpose/UI section:
    - **`ui/`**: Generic, reusable design components.
      ...
      - **`dialogs/`**: Standardized dialog wrapper templates (`CompressoDialogMenu`, etc.).
...
- **Standard Wrappers**: **PREFERRED**. Use the pre-built wrappers in `src/components/ui/dialogs/` to ensure consistent behavior across desktop and mobile.
  - **`CompressoDialogMenu`**: For side-menus. Slides from right (desktop) or left (mobile). Supports an `action` prop for a header button (e.g., Save).
  - **`CompressoDialogPopup`**: For standard modals/forms. Centered (desktop) or left-slide full-width (mobile).
  - **`CompressoDialogWindow`**: For large floating windows. Slides from right with buffer (desktop) or left-slide full-width (mobile).
- **Usage Example**:
  ```html
  <CompressoDialogPopup title="My Dialog" @hide="onHide">
    <!-- Your Content Here -->
  </CompressoDialogPopup>
  ```
- **Header Styling**: Dialog headers in `src/components/ui/dialogs/` should use `text-primary` for title text.
- **Header Actions**: `CompressoDialogMenu` and `CompressoDialogWindow` use an `actions` array prop (objects with `label`, `icon`, `color`, `emit`, `tooltip`, `disabled`). Place actions adjacent to the close arrow.
...
- **Confirm Dialogs**: Use `src/components/ui/dialogs/CompressoDialogConfirm.vue` for confirmation prompts instead of `$q.dialog`. Pass `title`, `message`, `okLabel`, `okColor`, and `cancelColor` props as needed, then call `confirmDialogRef.show()` via a `ref`.
...
- **Standard Components**: **PREFERRED**. Use the pre-built buttons in `src/components/ui/buttons/` to ensure consistent styling.
  - **`CompressoListActionButton`**: For inline list actions (e.g., Edit, Delete). Replaces `flat dense round`.
  - **`CompressoListAddButton`**: For "Add" actions in list headers. Replaces `outline dense round` with `icon="add"`.
  - **`CompressoSettingsSubmitButton`**: For form submit buttons. Replaces `outline full-width` submit buttons.
...
- **Dropdown Buttons**:
  ...
  - **Always** apply `content-class="compresso-dropdown-menu"` to ensure consistent menu styling (borders, shadows).
- **List Icon Actions**: (Use `CompressoListActionButton`)
  - **Info**: `icon="info"`, `color="info"`.
  - **Settings/Config**: `icon="tune"`, `color="grey-8"`.
  - **Delete**: `icon="delete"`, `color="negative"`; if disabled, use `color="grey-6"`.
- **List Add Actions**: (Use `CompressoListAddButton`)
- **Text-Only Actions**: Use `outline` with `color="secondary"` for buttons that only display text (e.g., Cancel).
- **Submit Actions**: (Use `CompressoSettingsSubmitButton`)
...
- **Compresso Plugin Ecosystem**:
  - Compresso plugin forks store build metadata in a **`repo` branch**, specifically in a **`repo.json`** file.
