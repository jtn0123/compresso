...
export function useSystemStatus() {
  const systemInfo = ref<SystemStatus | null>(null)
  const liveMetrics = ref<LiveSystemMetrics>({
    cpu_percent: 0,
    memory_percent: 0,
    memory_used_gb: 0,
    disk_percent: 0,
  ...
  const gpuHistory = ref<GpuHistory>({})

  async function fetchSystemInfo() {
    try {
      const response = await axios.get<SystemStatus>(getCompressoApiUrl('v2', 'system/status'))
  ...
  function startLiveMetrics(ws: WebSocket | null | undefined): void {
    if (ws) {
      ws.send(JSON.stringify({ command: 'start_system_status', params: {} }))
    }
  }

  function stopLiveMetrics(ws: WebSocket | null | undefined): void {
    if (ws) {
      ws.send(JSON.stringify({ command: 'stop_system_status', params: {} }))
    }
  }

  function updateLiveMetrics(data: LiveSystemMetricsMessage): void {
