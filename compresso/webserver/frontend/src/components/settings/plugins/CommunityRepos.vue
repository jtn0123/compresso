<template>
  <CompressoDialogMenu ref="dialogRef" :title="$t('components.plugins.communityRepos')" @hide="onDialogHide">
    <!-- CONTENT -->
    <div class="column full-height">
      ...
      <div v-else-if="error" class="q-pa-md text-center text-negative">
        ...
        <div>{{ error }}</div>
        <q-btn color="secondary" class="q-mt-md" :label="$t('buttons.retry')" @click="fetchForks" />
      </div>
      ...
      <q-list v-else class="q-pa-sm">
        <q-item v-for="repo in repos" :key="repo.id" class="shadow-1 rounded-borders q-mb-sm">
          <!-- ICON -->
          ...
          <q-item-section side top class="q-mt-sm">
            <q-btn round flat dense color="secondary" icon="add_circle" :aria-label="$t('a11y.addRepository')" @click="addRepo(repo)">
