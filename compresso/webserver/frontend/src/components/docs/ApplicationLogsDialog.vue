...
const initCompressoWebsocket = () => {
  compressoWSHandler = CompressoWebsocketHandler($t)
  ws = compressoWSHandler.init()

  compressoWSHandler.addEventListener('open', 'start_system_logs', () => {
    requestSystemLogs()
  })
...
const closeCompressoWebsocket = () => {
  if (compressoWSHandler) {
    compressoWSHandler.close()
    compressoWSHandler = null
  }
  ws = null
}
