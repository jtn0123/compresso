import { mount, shallowMount, type MountingOptions, type Stubs } from '@vue/test-utils'
import { createI18n } from 'vue-i18n'
import type { Component } from 'vue'

type StubRecord = Exclude<Stubs, string[]>
type HelperMountOptions = MountingOptions<Record<string, unknown>> & {
  stubs?: StubRecord
}

function stubsAsRecord(stubs: Stubs | undefined): StubRecord {
  if (!stubs) return {}
  if (Array.isArray(stubs)) return Object.fromEntries(stubs.map((name) => [name, true]))
  return stubs
}
