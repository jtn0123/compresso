<template>
  <q-page padding>
    <div class="q-pa-md">
      ...
      <div class="row q-col-gutter-md q-mb-lg">
          ...
          <q-card flat bordered class="stat-card stat-card--primary">
            <q-card-section>
              <div class="stat-label">
                {{ $t('pages.compressionDashboard.totalSpaceSaved')
                }}<q-tooltip>{{ $t('pages.compressionDashboard.tooltipSpaceSaved') }}</q-tooltip>
              </div>
        ...
        <div class="col-12 col-sm-6 col-md-3">
          <q-card flat bordered class="stat-card stat-card--accent">
            <q-card-section>
              <div class="stat-label">
                {{ $t('pages.compressionDashboard.avgCompressionRatio')
                }}<q-tooltip>{{ $t('pages.compressionDashboard.tooltipRatio') }}</q-tooltip>
              </div>
              <div class="stat-value">
        ...
        <div class="col-12 col-sm-6 col-md-3">
          <q-card flat bordered class="stat-card stat-card--info">
            <q-card-section>
              <div class="stat-label">
                {{ $t('pages.compressionDashboard.pendingEstimate')
                }}<q-tooltip>{{ $t('pages.compressionDashboard.tooltipPendingEstimate') }}</q-tooltip>
              </div>
              <div class="stat-value">
                <q-skeleton v-if="loadingSummary" type="text" width="60px" />
              ...
              <div class="stat-sublabel">
                <template v-if="!loadingSummary">{{
                  $t('pages.compressionDashboard.filesCount', { count: pendingEstimate.pending_count })
                }}</template>
              </div>
            </q-card-section>
          </q-card>
      ...
      <!-- Charts Section -->
      <div class="text-overline text-grey-6 q-mb-sm q-mt-sm">
        {{ $t('pages.compressionDashboard.sectionDistribution') }}
      </div>
      <div class="row q-col-gutter-md q-mb-lg">
        <div class="col-12 col-md-6">
          ...
          <q-card v-else>
            <q-card-section class="text-center text-grey">{{
              $t('pages.compressionDashboard.noCodecData')
            }}</q-card-section>
          </q-card>
        </div>
        <div class="col-12 col-md-6">
          ...
          <q-card v-else>
            <q-card-section class="text-center text-grey">{{
              $t('pages.compressionDashboard.noResolutionData')
            }}</q-card-section>
          </q-card>
        </div>
      </div>
      ...
      <div class="row q-col-gutter-md q-mb-lg">
        <div class="col-12 col-md-6">
          ...
          <q-card v-else>
            <q-card-section class="text-center text-grey">{{
              $t('pages.compressionDashboard.noTimelineData')
            }}</q-card-section>
          </q-card>
        </div>
        <div class="col-12 col-md-6">
          <EncodingSpeedChart :data="encodingSpeedData" :loading="chartsLoading" />
        </div>
      </div>
      ...
      <q-card class="q-mb-lg">
        <q-card-section>
          ...
          <div v-if="!selectedLibraryId" class="text-caption text-grey q-mt-sm">Select a library to analyze</div>
        </q-card-section>
        ...
        <q-card-section v-if="analysisStatus === 'running'">
          <div class="text-caption q-mb-xs">
            {{
              $t('flow.analysisProgress', {
                checked: analysisProgress.checked || 0,
        ...
        <template v-if="analysisResults && analysisResults.groups">
          <q-card-section>
            ...
            >
              ...
              <template #body-cell-estimated_savings="cellProps">
                <q-td :cell-props="cellProps">
                  ...
                  <template v-else>
                    {{ cellProps.row.estimated_savings_pct }}% (~{{
                      formatBytes(cellProps.row.estimated_savings_bytes)
                    }})
                  </template>
                </q-td>
              </template>
          ...
          <q-card-section v-if="analysisResults.last_run" class="text-caption text-grey">
            {{ $t('flow.lastAnalyzed') }}: {{ analysisResults.last_run }} — {{ $t('flow.estimatesLive') }}
          </q-card-section>
        </template>
      </q-card>
        ...
        <q-card-section>
          ...
          >
            <template #body-cell-ratio="props">
              <q-td :props="props">
                <q-badge :color="props.row.ratio < 1 ? 'positive' : 'negative'">
                  {{ (props.row.ratio * 100).toFixed(1) }}%
                </q-badge>
              </q-td>
            </template>
            <template #body-cell-space_saved="props">
              <q-td :props="props">
  ...
  setup() {
    ...
    const analysisColumns = [
      ...
      },
      {
        ...
        label: t('flow.analysisEstSavings'),
        field: 'estimated_savings_pct',
        align: 'right',
      },
    ...
    const libraryColumns = [
      ...
      {
        name: 'file_count',
        label: t('pages.compressionDashboard.columnFiles'),
        field: 'file_count',
        align: 'right',
        sortable: true,
      },
      {
        name: 'total_source_size',
    ...
    const fileColumns = [
      ...
      {
        name: 'source_size',
        label: t('pages.compressionDashboard.columnOriginal'),
        field: 'source_size',
        align: 'right',
        sortable: true,
        format: (v) => formatBytes(v),
      ...
      {
        name: 'ratio',
        label: t('pages.compressionDashboard.columnRatio'),
        field: 'ratio',
        align: 'center',
        sortable: true,
      },
      {
        name: 'space_saved',
        label: t('pages.compressionDashboard.columnSaved'),
        field: 'space_saved',
        align: 'right',
        sortable: true,
      },
      {
        name: 'source_codec',
        label: t('pages.compressionDashboard.columnSourceCodec'),
        field: 'source_codec',
        align: 'left',
      },
      {
        name: 'destination_codec',
        label: t('pages.compressionDashboard.columnDestCodec'),
        field: 'destination_codec',
        align: 'left',
      },
      {
        name: 'library_id',
        label: t('pages.compressionDashboard.columnLibrary'),
        field: 'library_id',
        align: 'center',
      },
      {
        name: 'finish_time',
        label: t('pages.compressionDashboard.columnDate'),
        field: 'finish_time',
        align: 'left',
        sortable: true,
      },
    ...
    async function loadCharts() {
      ...
      try {
        ...
        const results = await Promise.allSettled([
          ...
          axios.get(
            getCompressoApiUrl('v2', 'compression/timeline') +
              param +
              (param ? '&' : '?') +
              'interval=' +
        ...
        if (results[1].status === 'fulfilled' && results[1].value.data)
          resolutionData.value = results[1].value.data.resolutions || []
        if (results[2].status === 'fulfilled' && results[2].value.data)
          timelineData.value = results[2].value.data.data || []
        if (results[3].status === 'fulfilled' && results[3].value.data)
          encodingSpeedData.value = results[3].value.data.data || []
      } catch (error) {
