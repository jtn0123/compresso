...
def install_bundled_plugins(plugins_path: str | os.PathLike[str]) -> None:
    ...
    for entry in os.listdir(BUNDLED_PLUGINS_DIR):
        _install_bundled_plugin_entry(plugins_path, entry)


def _install_bundled_plugin_entry(plugins_path: str | os.PathLike[str], entry: str) -> None:
    try:
    ...
    if not os.path.exists(info_file):
        return
    with open(info_file) as file_handle:
        bundled_info = narrowing.mapping_value(json.load(file_handle))
    try:
        ...
        return
    if _bundled_plugin_needs_install(target_dir, bundled_info):
...
def _bundled_plugin_needs_install(target_dir: str, bundled_info: Mapping[str, object]) -> bool:
    if not os.path.exists(target_dir):
        return True
    target_info_file = os.path.join(target_dir, "info.json")
    if not os.path.exists(target_info_file):
        return True
    with open(target_info_file) as file_handle:
        existing_info = narrowing.mapping_value(json.load(file_handle))
    return _version_newer(bundled_info.get("version", "0.0.0"), existing_info.get("version", "0.0.0"))
