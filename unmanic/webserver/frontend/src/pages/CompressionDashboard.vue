...
export default {
  ...
  setup() {
    ...
    async function loadSummary() {
      try {
        const response = await axios.get(getUnmanicApiUrl('v2', 'compression/summary') + buildLibraryParam());
        if (response.data) {
    ...
    async function loadPendingEstimate() {
      try {
        const response = await axios.get(getUnmanicApiUrl('v2', 'compression/pending-estimate'));
        if (response.data) {
    ...
    async function loadCharts() {
      ...
      try {
        ...
        const results = await Promise.allSettled([
          axios.get(getUnmanicApiUrl('v2', 'compression/codec-distribution') + param),
          axios.get(getUnmanicApiUrl('v2', 'compression/resolution-distribution') + param),
          axios.get(getUnmanicApiUrl('v2', 'compression/timeline') + param + (param ? '&' : '?') + 'interval=' + timelineInterval.value),
        ]);
    ...
    async function loadStats() {
      ...
      try {
        ...
        const response = await axios.post(getUnmanicApiUrl('v2', 'compression/stats'), {
          start: start,
