...
skip_if_no_fixture = pytest.mark.skipif(
    not os.path.exists(FIXTURE_PATH), reason="Test fixture tests/fixtures/sample_30s.mp4 not found"
)

skip_if_no_ffprobe = pytest.mark.skipif(
    shutil.which("ffprobe") is None, reason="ffprobe executable is required for metadata integration tests"
)
