...
Unit tests for compresso.libs.healthcheck.HealthCheckManager.
...
class TestHealthCheckQuickCheck:
    ...
    @patch("compresso.libs.healthcheck.os.path.exists", return_value=False)
    ...
    @patch("compresso.libs.healthcheck.probe_file", return_value=None)
...
class TestHealthCheckThoroughCheck:
    ...
    @patch("compresso.libs.healthcheck.subprocess.run")
    @patch("compresso.libs.healthcheck.os.path.exists", return_value=True)
...
class TestHealthCheckCheckFile:
    ...
    def setup_class(self):
        self.config_path = tempfile.mkdtemp(prefix="compresso_tests_healthcheck_")
    ...
    @patch.object(
        __import__("compresso.libs.healthcheck", fromlist=["HealthCheckManager"]).HealthCheckManager,
        "quick_check",
        return_value=(True, ""),
    )
    ...
    @patch.object(
        __import__("compresso.libs.healthcheck", fromlist=["HealthCheckManager"]).HealthCheckManager,
        "quick_check",
        return_value=(False, "No streams found in file"),
    )
    ...
    @patch.object(
        __import__("compresso.libs.healthcheck", fromlist=["HealthCheckManager"]).HealthCheckManager,
        "thorough_check",
        return_value=("warning", "5 warning(s): line1"),
    )
...
class TestHealthSummary:
    ...
    def setup_class(self):
        self.config_path = tempfile.mkdtemp(prefix="compresso_tests_health_summary_")
...
class TestHealthStatusesPaginated:
    ...
    def setup_class(self):
        self.config_path = tempfile.mkdtemp(prefix="compresso_tests_health_pag_")
    ...
    def test_invalid_order_column_falls_back_to_last_checked(self):
        ...
        mgr = self._make_manager()
        result = mgr.get_health_statuses_paginated(order={"column": "DROP TABLE", "dir": "desc"})
    ...
    def test_valid_order_column_works(self):
        ...
        mgr = self._make_manager()
        result = mgr.get_health_statuses_paginated(order={"column": "abspath", "dir": "asc"})
    ...
    def test_result_rows_have_expected_keys(self):
        ...
        expected_keys = ["id", "abspath", "library_id", "status", "check_mode", "error_detail", "last_checked", "error_count"]
...
class TestScheduleLibraryScan:
    ...
    @patch("compresso.libs.healthcheck.threading.Thread")
...
class TestHistoryOrderWhitelist:
    ...
    def setup_class(self):
        self.config_path = tempfile.mkdtemp(prefix="compresso_tests_history_order_")
    ...
    def test_invalid_column_does_not_crash(self):
        ...
        history = History()
        result = history.get_historic_task_list_filtered_and_sorted(order={"column": "__class__", "dir": "asc"})
...
class TestRecordsTotalRespectsLibraryId:
    ...
    def setup_class(self):
        self.config_path = tempfile.mkdtemp(prefix="compresso_tests_records_total_")
...
class TestGetScanProgressDeepCopy:
    ...
    def test_modifying_returned_workers_does_not_affect_original(self):
        ...
        with HealthCheckManager._lock:
            HealthCheckManager._scan_progress["workers"] = {0: {"status": "checking", "current_file": "/test.mkv"}}
...
class TestFileLocksClearedAfterScan:
    ...
    @patch("compresso.libs.healthcheck.HealthCheckManager.check_file")
    def test_file_locks_cleared_after_scan(self, mock_check):
        ...
        with patch("compresso.libs.healthcheck.os.walk", return_value=[]):
            mock_lib = MagicMock()
            mock_lib.path = "/empty/library"
            with patch("compresso.libs.unmodels.Libraries") as mock_libs:
