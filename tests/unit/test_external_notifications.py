...
class TestWebhookGeneric:
    ...
    def test_webhook_sends_custom_headers(self, mock_post):
        ...
        channel = _make_channel(channel_type="webhook", url="https://example.com/hook", headers=custom_headers)
...
class TestTriggerFiltering:
    ...
    def test_dispatch_only_sends_to_matching_channels(self, mock_get_channels, mock_post):
        mock_post.return_value = MagicMock(status_code=200)
        discord_ch = _make_channel(channel_type="discord", name="discord-ch", triggers=["task_completed"])
        _make_channel(
            channel_type="slack", name="slack-ch", url="https://hooks.slack.com/services/T00/B00/xxx", triggers=["task_failed"]
        )
    ...
    def test_get_channels_for_event_filters_by_trigger(self):
        ...
        with (
            patch(
                "compresso.libs.external_notifications.ExternalNotificationDispatcher._get_channels_for_event",
                wraps=dispatcher._get_channels_for_event,
            ),
            patch("compresso.config.Config") as MockConfig,
        ):
