...
class TestManageEventSchedulesExtended:
    def _setup_foreman_with_schedule(self, repetition, day_of_week, time_now, schedule_time=None):
        ...
        with patch("compresso.libs.foreman.WorkerGroup") as mock_wg_cls, patch("compresso.libs.foreman.datetime") as mock_dt:
    ...
    def test_skips_when_time_matches_last_run(self):
        ...
        foreman.run_task = MagicMock()
        with patch("compresso.libs.foreman.WorkerGroup"), patch("compresso.libs.foreman.datetime") as mock_dt:
            mock_today = MagicMock()
...
class TestInitWorkerThreads:
    ...
    def test_spawns_new_workers(self, mock_wg_cls):
        ...
        mock_wg_cls.get_all_worker_groups.return_value = [{"id": "g1", "name": "Default", "number_of_workers": 2}]
