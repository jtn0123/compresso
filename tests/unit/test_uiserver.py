...
def _make_uiserver(developer=False):
    ...
    with (
        patch("compresso.libs.uiserver.config.Config") as mock_config,
...
class TestUIServerSetLogging:
    ...
    def test_set_logging_with_path_non_developer(self, mock_get_logger, mock_handler_cls, mock_exists):
        server = _make_uiserver(developer=False)
    ...
    @patch("compresso.libs.uiserver.logging.handlers.TimedRotatingFileHandler")
    @patch("compresso.libs.uiserver.logging.getLogger")
    def test_set_logging_creates_directory(self, mock_get_logger, mock_handler_cls, mock_makedirs, mock_exists):
        server = _make_uiserver(developer=True)
...
class TestUIServerMakeWebApp:
    ...
    def test_make_web_app_developer_mode(self, mock_swagger, mock_makedirs):
        ...
        with (
            patch("compresso.libs.uiserver.tornado.web.Application") as mock_app_cls,
            patch.dict(
                "sys.modules",
                {
