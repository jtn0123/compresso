...
from unmanic.webserver.api_v2.compression_api import ApiCompressionHandler

VALIDATE_LIB = 'unmanic.webserver.api_v2.compression_api.validate_library_exists'
...
class TestCompressionApiStats(ApiTestBase):
    ...
    @patch('unmanic.webserver.helpers.compression_stats.get_compression_stats_paginated')
    ...
    def test_get_stats_invalid_json(self):
        resp = self.fetch(
            '/unmanic/api/v2/compression/stats',
            method='POST',
            body='not json',
...
class TestCompressionApiSummary(ApiTestBase):
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_compression_summary')
...
class TestCompressionApiDistributions(ApiTestBase):
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_codec_distribution')
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_resolution_distribution')
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_container_distribution')
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_pending_estimate')
...
class TestCompressionApiTimeline(ApiTestBase):
    ...
    @patch(VALIDATE_LIB, return_value=True)
    @patch('unmanic.webserver.helpers.compression_stats.get_space_saved_over_time')
