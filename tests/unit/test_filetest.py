...
def _make_filetest():
    ...
    with patch('unmanic.libs.filetest.config.Config'), \
         patch('unmanic.libs.filetest.UnmanicLogging'), \
         patch('unmanic.libs.filetest.PluginsHandler') as mock_ph:
...
class TestFileInUnmanicIgnoreLockfile:
    ...
    def test_exact_match_returns_true(self, tmp_path):
        ignore_file = tmp_path / '.unmanicignore'
        ignore_file.write_text('video.mp4\n')
        path = str(tmp_path / 'video.mp4')

        ft = _make_filetest()
        assert ft.file_in_unmanic_ignore_lockfile(path) is True
    ...
    def test_substring_no_false_positive(self, tmp_path):
        ...
        ft = _make_filetest()
        assert ft.file_in_unmanic_ignore_lockfile(path) is False
...
class TestShouldFileBeAddedToTaskList:
    ...
    def test_returns_false_if_in_ignore_file(self):
        ft = _make_filetest()
        ft.file_in_unmanic_ignore_lockfile = MagicMock(return_value=True)
        ...
        assert any(i['id'] == 'unmanicignore' for i in issues)

    def test_returns_false_if_failed_in_history(self):
        ft = _make_filetest()
        ft.file_in_unmanic_ignore_lockfile = MagicMock(return_value=False)
