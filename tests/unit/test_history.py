...
class TestHistory:
    ...
    def _create_task(
        self, label="test_file.mkv", abspath=None, success=True, start_time=None, finish_time=None, worker="worker-0"
    ):
    ...
    def test_filtered_and_sorted_by_time_range(self):
        ...
        result = list(history.get_historic_task_list_filtered_and_sorted(after_time=after, before_time=before))
        assert len(result) == 1
    ...
    def test_filtered_and_sorted_order_asc(self):
        ...
        history = self._make_history()
        result = list(history.get_historic_task_list_filtered_and_sorted(order={"column": "task_label", "dir": "asc"}))
    ...
    def test_filtered_and_sorted_order_invalid_column_falls_back_to_id(self):
        ...
        result = list(
            history.get_historic_task_list_filtered_and_sorted(order={"column": "nonexistent_column", "dir": "desc"})
        )
        assert len(result) == 2
    ...
    def test_filtered_and_sorted_pagination(self):
        ...
        history = self._make_history()
        result = list(history.get_historic_task_list_filtered_and_sorted(start=1, length=2))
        assert len(result) == 2
    ...
    def test_with_source_probe_filter_by_search_and_success(self):
        ...
        history = self._make_history()
        result = list(history.get_historic_tasks_list_with_source_probe(search_value="movie", task_success=True))
        assert len(result) == 1
