...
def _make_handler():
    ...
    with (
        patch(f"{WS_MOD}.config.Config") as mock_config_cls,
...
class TestAsyncLoops:
    ...
    async def test_async_frontend_message_sends_data(self):
        ...
        with (
            patch(f"{WS_MOD}.FrontendPushMessages") as mock_fpm_cls,
    ...
    async def test_async_pending_tasks_info_sends_data(self):
        ...
        with (
            patch(f"{WS_MOD}.pending_tasks.prepare_filtered_pending_tasks") as mock_pending,
            patch(f"{WS_MOD}.estimate_queue_eta", return_value={"total_queue_eta_seconds": 120}),
            patch(f"{WS_MOD}.gen.sleep", new_callable=AsyncMock) as mock_sleep,
        ):
            mock_pending.return_value = {"results": [{"id": 1, "abspath": "/test.mkv", "priority": 100, "status": "pending"}]}
    ...
    async def test_async_completed_tasks_info_sends_data(self):
        ...
        with (
            patch(f"{WS_MOD}.completed_tasks.prepare_filtered_completed_tasks") as mock_completed,
            patch(f"{WS_MOD}.common.make_timestamp_human_readable", return_value="5 mins ago"),
            patch(f"{WS_MOD}.gen.sleep", new_callable=AsyncMock) as mock_sleep,
        ):
            mock_completed.return_value = {
                "results": [
                    {"id": 1, "task_label": "test.mkv", "task_success": True, "finish_time": str(int(time.time()) - 120)}
                ]
            }
    ...
    async def test_async_completed_tasks_just_now(self):
        ...
        with (
            ...
            mock_completed.return_value = {
                "results": [{"id": 1, "task_label": "recent.mkv", "task_success": True, "finish_time": str(int(time.time()))}]
            }
    ...
    async def test_async_system_status_sends_data(self):
        ...
        with (
            patch(f"{WS_MOD}.psutil") as mock_psutil,
...
class TestStreamRowLimits:
    ...
    def _run_stream(self, handler, coroutine):
        import asyncio
    ...
    def test_pending_stream_requests_bounded_page(self):
        ...
        with (
            patch(f"{WS_MOD}.pending_tasks.prepare_filtered_pending_tasks", side_effect=fake_prepare),
            patch(f"{WS_MOD}.estimate_queue_eta", return_value=None),
            patch(f"{WS_MOD}.gen.sleep", new=AsyncMock()),
        ):
            self._run_stream(handler, handler.async_pending_tasks_info())
    ...
    def test_completed_stream_requests_bounded_page(self):
        ...
        with (
            patch(f"{WS_MOD}.completed_tasks.prepare_filtered_completed_tasks", side_effect=fake_prepare),
            patch(f"{WS_MOD}.gen.sleep", new=AsyncMock()),
        ):
            self._run_stream(handler, handler.async_completed_tasks_info())
