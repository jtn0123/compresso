...
def _make_cli(pluginscli_mod, plugins_dir="/fake/plugins"):
    with (
        patch.object(pluginscli_mod, "config") as mock_config,
...
class TestParseRunnerInputs:
    ...
    def test_returns_none_for_none_input(self):
        pluginscli = _import_pluginscli()
        cli = _make_cli(pluginscli)
        result, errors = cli._parse_runner_inputs(None, {}, {})
        assert result is None
    ...
    def test_parses_by_runner_name(self):
        ...
        result, errors = cli._parse_runner_inputs("on_worker_process", {}, by_runner)
        assert errors == []
        assert len(result) == 1
    ...
    def test_parses_by_plugin_name(self):
        ...
        result, errors = cli._parse_runner_inputs("Worker", by_name, {})
        assert errors == []
        assert len(result) == 1
    ...
    def test_returns_error_for_invalid_token(self):
        pluginscli = _import_pluginscli()
        cli = _make_cli(pluginscli)
        result, errors = cli._parse_runner_inputs("nonexistent_runner", {}, {})
        assert result is None
    ...
    def test_handles_comma_separated(self):
        ...
        result, errors = cli._parse_runner_inputs("on_worker_process,on_postprocessor_file_movement", {}, by_runner)
        assert errors == []
        assert len(result) == 2
    ...
    def test_deduplicates_runners(self):
        ...
        result, errors = cli._parse_runner_inputs("on_worker_process,on_worker_process", {}, by_runner)
        assert errors == []
        assert len(result) == 1
...
class TestTestInstalledPlugins:
    def test_tests_specific_plugin(self, capsys):
        ...
        with (
            ...
            mock_ph = MagicMock()
            mock_ph.get_plugin_list_filtered_and_sorted.return_value = [{"plugin_id": "p1", "name": "Plugin1"}]
            mock_ph_cls.return_value = mock_ph
            mock_pe = MagicMock()
...
class TestCreateNewPlugins:
    def test_returns_when_no_plugin_details(self):
        ...
        cli = _make_cli(pluginscli)
        with (
            patch.object(cli, "_collect_new_plugin_details", return_value=(None, None)),
    ...
    def test_creates_new_plugin_files_when_details_exist(self):
        ...
        with (
            patch.object(cli, "_collect_new_plugin_details", return_value=(details, selected)),
            patch.object(cli, "create_new_plugin_files") as mock_create,
        ):
            cli.create_new_plugins()
...
class TestReloadPluginFromDisk:
    def test_reload_plugin_success(self, capsys):
        ...
        with (
            patch.object(pluginscli.PluginsCLI, "_PluginsCLI__get_installed_plugins", return_value=[plugin_result]),
...
class TestRemovePluginInteractive:
    def test_remove_plugin_returns_on_go_back(self):
        ...
        cli = _make_cli(pluginscli)
        with (
            patch.object(pluginscli.inquirer, "prompt", return_value={"cli_action": "Go Back"}),
            patch.object(
                pluginscli.PluginsCLI, "_PluginsCLI__get_installed_plugins", return_value=[{"plugin_id": "p1", "id": 1}]
            ),
    ...
    def test_remove_plugin_uninstalls_selected_plugin(self):
        ...
        cli = _make_cli(pluginscli)
        with (
            patch.object(pluginscli.inquirer, "prompt", return_value={"cli_action": "p1"}),
            patch.object(
                pluginscli.PluginsCLI, "_PluginsCLI__get_installed_plugins", return_value=[{"plugin_id": "p1", "id": 42}]
            ),
            patch.object(cli, "_uninstall_plugin_by_db_table_id") as mock_uninstall,
        ):
            cli.remove_plugin()
...
class TestConfigureAndInstallTestData:
    def test_configure_test_data_updates_modifiers(self):
        ...
        cli = _make_cli(pluginscli)
        with (
            patch.object(pluginscli.inquirer, "prompt", return_value={"selected_file": "sample.mp4"}),
    ...
    def test_install_test_data_downloads_and_copies_files(self):
        ...
        with (
            patch.object(pluginscli.os.path, "exists", return_value=False),
