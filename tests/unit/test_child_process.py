...
class TestKillAllPluginProcesses:
    def test_kills_registered_processes(self):
        ...
        with (
            patch("compresso.libs.unplugins.child_process.psutil.Process", return_value=mock_proc),
    ...
    def test_handles_no_such_process(self):
        ...
        with patch("compresso.libs.unplugins.child_process.psutil.Process", side_effect=psutil.NoSuchProcess(300)):
            # Should not raise
            kill_all_plugin_processes()
    ...
    def test_force_kills_alive_processes(self):
        ...
        with (
            patch("compresso.libs.unplugins.child_process.psutil.Process", return_value=mock_proc),
            patch("compresso.libs.unplugins.child_process.psutil.wait_procs", side_effect=[([], [alive_proc]), ([], [])]),
        ):
            kill_all_plugin_processes()
...
class TestPluginChildProcessRun:
    def test_run_launches_process_and_returns(self):
        ...
        with patch("compresso.libs.unplugins.child_process.CompressoLogging") as mock_log:
            ...
            with patch("multiprocessing.Process", return_value=mock_proc), patch.object(pcp, "_monitor", return_value=True):
                target_fn = MagicMock()
